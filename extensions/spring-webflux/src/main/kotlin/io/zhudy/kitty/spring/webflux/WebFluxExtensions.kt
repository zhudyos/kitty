...
import io.zhudy.kitty.core.util.TracingUtils.HTTP_HEADER_REQUEST_ID
...
private fun requestStringParam(request: ServerRequest, where: String, name: String): String {
    return requestParam(request, where, name) ?: throw MissingRequestParameterException(where, name)
}

private fun requestTrimStringParam(request: ServerRequest, where: String, name: String): String {
    return requestParam(request, where, name)?.trim() ?: throw MissingRequestParameterException(where, name)
}

private fun requestParam(request: ServerRequest, where: String, name: String): String? {
    return if ("path" == where) request.pathVariables()[name] else request.queryParam(name).orElse(null)
}